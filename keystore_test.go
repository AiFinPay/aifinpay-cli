...
func TestEncryptedKeystoreRoundTripContainsNoPlaintextSecret(t *testing.T) {
	...
	if err := saveKeystore(secret); err != nil { t.Fatal(err) }
	...
	if err := json.Unmarshal(raw, &ks); err != nil { t.Fatal(err) }
...
func TestKeystorePermissionsAreRepaired(t *testing.T) {
	...
	if err := os.Chmod(filepath.Dir(path), 0o755); err != nil { t.Fatal(err) }
	if got := loadKeystoreSecret(); got != secret { t.Fatal("failed to reload keystore") }
	fileInfo, _ := os.Stat(path)
	dirInfo, _ := os.Stat(filepath.Dir(path))
	if fileInfo.Mode().Perm() != 0o600 { t.Fatalf("file mode = %o", fileInfo.Mode().Perm()) }
	if dirInfo.Mode().Perm() != 0o700 { t.Fatalf("dir mode = %o", dirInfo.Mode().Perm()) }
}
...
func TestLegacyPlaintextKeystoreMigratesOnRead(t *testing.T) {
	...
	if err := os.WriteFile(path, legacy, 0o600); err != nil { t.Fatal(err) }

	if got := loadKeystoreSecret(); got != secret { t.Fatal("legacy migration lost secret") }
	raw, err := os.ReadFile(path)
	if err != nil { t.Fatal(err) }
	if strings.Contains(string(raw), secret) || strings.Contains(string(raw), `"secret"`) {
...
func TestPBKDF2IsDeterministicAndSaltSensitive(t *testing.T) {
	...
	if string(a) != string(b) { t.Fatal("PBKDF2 is not deterministic") }
	if string(a) == string(c) { t.Fatal("PBKDF2 ignored salt") }
}
